...
void subscription_callback(const void * msvin) {
  ...
  // Get linear x (ranges typically from -1.0 to 1.0 in ROS)

  float val_x = msg->linear.x;



  // Constrain input to avoid over-driving

  if(val_x > 1.0) val_x = 1.0;

  if(val_x < -1.0) val_x = -1.0;



  // Map -1.0...1.0 to PWM range

  int motorOutput = neutral + (int)(val_x * range);
