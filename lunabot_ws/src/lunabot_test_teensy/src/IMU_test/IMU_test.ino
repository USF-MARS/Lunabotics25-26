...
/*
  HOW TO WIRE THIS TO YOUR ESP32:
  IMU (HWT905)      ESP32 (Using Hardware Serial2)
    VCC <--------->  3.3V (CRITICAL: ESP32 is 3.3V logic only!)
    TX  <--------->  RX2 (GPIO 16) --> IMU talks, ESP32 listens
    RX  <--------->  TX2 (GPIO 17) --> ESP32 talks, IMU listens
    GND <--------->  GND
*/

// Explicitly define the ESP32 pins for Serial2 to avoid board-specific conflicts
#define RX2_PIN 16
#define TX2_PIN 17
...
void loop() {
    // Read from the IMU via Serial2
    while (Serial2.available())
    {
      WitSerialDataIn(Serial2.read());
    }

    // Read commands from the PC Serial Monitor
    ...
    // If new data arrived from the IMU, process and print it
    if(s_cDataUpdate)
    {
      for(i = 0; i < 3; i++)
      {
      ...
      if(s_cDataUpdate & ACC_UPDATE)
      {
        ...
        Serial.print(fAcc[2], 3); Serial.print("\r\n");
      ...
      if(s_cDataUpdate & GYRO_UPDATE)
      {
        ...
        Serial.print(fGyro[2], 1); Serial.print("\r\n");
      ...
      if(s_cDataUpdate & ANGLE_UPDATE)
      {
        ...
        Serial.print(fAngle[2], 3); Serial.print("\r\n");
      ...
      {
        Serial.print("mag:");
        Serial.print(sReg[HX]); Serial.print(" ");
        Serial.print(sReg[HY]); Serial.print(" ");
        Serial.print(sReg[HZ]); Serial.print("\r\n");
...
static void CmdProcess(void)
{
  switch(s_cCmd)
  {
    ...
    case 'B':  if(WitSetUartBaud(WIT_BAUD_115200) != WIT_HAL_OK) Serial.print("\r\nSet Baud Error\r\n");
              ...
              {
                // Updated for ESP32 with explicit pins
                Serial2.begin(c_uiBaud[WIT_BAUD_115200], SERIAL_8N1, RX2_PIN, TX2_PIN);
    ...
    case 'b':  if(WitSetUartBaud(WIT_BAUD_9600) != WIT_HAL_OK) Serial.print("\r\nSet Baud Error\r\n");
              ...
              {
                // Updated for ESP32 with explicit pins
                Serial2.begin(c_uiBaud[WIT_BAUD_9600], SERIAL_8N1, RX2_PIN, TX2_PIN); 
...
static void AutoScanSensor(void)
{
  ...
  {
    // Updated for ESP32: Explicitly passing SERIAL_8N1 and the RX/TX pins
    Serial2.begin(c_uiBaud[i], SERIAL_8N1, RX2_PIN, TX2_PIN);
    Serial2.flush();
    iRetry = 2; 
    s_cDataUpdate = 0;
    do
