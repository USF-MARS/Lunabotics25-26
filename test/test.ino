...
Sabertooth ST(128);
Sabertooth ST2(129); // The Sabertooth is on address 128. We'll name its object ST.
...
{
  Serial.begin(115200);
  SabertoothTXPinSerial.begin(38400); // 9600 is the default baud rate for Sabertooth packet serial.
  ST.autobaud();
  ST2.autobaud(); // Send the autobaud command to the Sabertooth controller(s).
...
void loop()
{
  ...
  for (power = 127; power >= -127; power --)
  {
    ...
    ST.motor(2,power);
    ST2.motor(1, power);
    ST2.motor(2,power); // Tip for SyRen users: Typing ST.motor(power) does the same thing as ST.motor(1, power).
    delay(200);          //                      Since SyRen doesn't have a motor 2, this alternative can save you typing.
  }
}